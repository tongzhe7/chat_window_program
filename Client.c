...
static void send_button_callback_func(GtkWidget *widget, gpointer callback_data)
{
    ...
    GtkTextIter		text_view_start;
    GtkTextIter		text_view_end;

    gtk_text_buffer_get_bounds(data->input_view_buffer, &text_view_start, &text_view_end);
    gchar *buffer;
    buffer = gtk_text_buffer_get_text(data->input_view_buffer, &text_view_start, &text_view_end, FALSE);
    ...
    gtk_text_buffer_set_text(data->input_view_buffer, "", -1);
}
...
static void *recv_and_show_message(void *arg)
{
    ...
    for (;;) {
        ...
        gtk_text_buffer_get_end_iter(data->show_view_buffer, &iter_tmp);
        gtk_text_buffer_insert(data->show_view_buffer, &iter_tmp, buffer, strlen(buffer));
    }
